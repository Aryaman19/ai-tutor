...
import type {
  UnifiedAudioResult,
  NarrationSegment,
} from "@ai-tutor/utils/src/audio/unified-audio-engine";
...
export const UnifiedPlayer: React.FC<UnifiedPlayerProps> = ({
  ...
  useEffect(() => {
    if (
      ...
      if (
        !unifiedAudioResult.audioUrl ||
        unifiedAudioResult.audioUrl.trim() === ""
      ) {
      ...
      if (!isValidUrl) {
        const error = new Error(
          `Invalid audio URL format: ${unifiedAudioResult.audioUrl}`
        );
  ...
  useEffect(() => {
    const ready =
      isAudioLoaded && canvasStates.length > 0 && audioEngine && layoutEngine;
    setIsReady(!!ready);
            ...
            <div className="mt-4 space-y-1 text-xs text-gray-500">
              ...
              <div>
                ...
                {canvasStates.length > 0
                  ? `✅ ${canvasStates.length} states`
                  : "⏳ Processing"}
              </div>
              <div>
                Engines:{" "}
                {audioEngine && layoutEngine ? "✅ Ready" : "⏳ Initializing"}
              </div>
              {unifiedAudioResult && (
                <div className="mt-2 text-xs text-blue-600">
                  Audio URL: {unifiedAudioResult.audioUrl ? "✅" : "❌"} | ID:{" "}
                  {unifiedAudioResult.audioId}
                </div>
              )}
            </div>
          </div>
